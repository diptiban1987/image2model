...
class Hitem3DAPI:
    ...
    async def _fetch_access_token(self) -> str:
        if not self.client_id or not self.client_secret:
            raise Exception("Hitem3D client_id/client_secret required to obtain access token")
    ...
    async def _request(self, method: str, url: str, **kwargs) -> httpx.Response:
        ...
        response = await self.client.request(method, url, headers={**headers, **auth_headers}, **kwargs)
    ...
    async def create_task(
        ...
        if result.get("code") != 200:
            ...
            if result.get("code") == 30010000 or "balance" in str(msg).lower():
                raise InsufficientBalanceError("Hitem3D balance is not enough. Please credit your account to generate.")
    ...
    async def wait_for_completion(
        ...
        while time.time() - start_time < max_wait_time:
            ...
            state = status.get("state") or status.get("status") or status.get("task_status") or status.get("taskState")
            if isinstance(state, (int, float)):
                ...
                if s in {"success", "succeeded", "completed", "complete", "finish", "finished", "done", "ok"}:
                    return status
                if s in {"failed", "error", "cancelled", "timeout"}:
                    raise Exception(f"Task failed: {status.get('error', 'Unknown error')}")
            if any(k in status and status.get(k) for k in ("url", "download_url", "obj_url", "stl_url", "glb_url", "fbx_url", "usdz_url")):
                return status
    ...
    async def generate_3d_model(
        ...
        **kwargs
        ...
        final_status = await self.wait_for_completion(task_id, poll_interval=poll_interval, max_wait_time=max_wait_time)
        ...
        for key, kext in (("obj_url", "obj"), ("stl_url", "stl"), ("glb_url", "glb"), ("fbx_url", "fbx"), ("usdz_url", "usdz")):
    ...
    for path in Path(output_dir).glob("*"):
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(data)) as zf:
                names = [n for n in zf.namelist() if not n.endswith("/")]
                preferred = [n for n in names if n.lower().endswith(path.suffix.lower())]
                target = preferred[0] if preferred else (names[0] if names else None)
                if not target:
...
def _main() -> None:
    ...
    if args.cmd == "repair-output":
        ...
        print(f"fixed={result['fixed']} skipped={result['skipped']} failed={result['failed']}")
