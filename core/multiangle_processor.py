...
class MultiAngleProcessor:
    ...
    def load_images(self, image_paths: List[str]) -> bool:
        ...
        if len(image_paths) < self.config.min_images:
            raise ValueError(f"Need at least {self.config.min_images} images, got {len(image_paths)}")
        
        if len(image_paths) > self.config.max_images:
            raise ValueError(f"Maximum {self.config.max_images} images allowed, got {len(image_paths)}")
    ...
    def _estimate_relative_pose(self, img1: np.ndarray, img2: np.ndarray) -> np.ndarray:
        ...
        kp1, desc1 = detector.detectAndCompute(cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY), None)
        kp2, desc2 = detector.detectAndCompute(cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY), None)
    ...
    def process_individual_meshes(self, base_name: str, output_dir: str) -> List[Dict[str, Any]]:
        """
        ...
        for i, (image_path, pose) in enumerate(zip(self.image_paths, self.estimated_poses)):
    ...
    def generate_consensus_texture(self, base_mesh: o3d.geometry.TriangleMesh) -> np.ndarray:
        """
...
def run_multiangle_pipeline(
    ...
    **kwargs
    ...
    try:
        ...
        print(f"Multi-angle processing completed in {result['stats']['total_seconds']}s")
...
if __name__ == "__main__":
    ...
    result = run_multiangle_pipeline(test_images, "test_model", "output")
    print(result)