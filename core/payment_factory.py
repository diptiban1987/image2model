...
from config.payment_config import (
    ...
    payment_settings, pricing_config
)
...
class PaymentProcessor:
    ...
    def _get_provider_class(self, provider: PaymentProvider) -> Optional[Type[BasePaymentProvider]]:
    ...
    async def validate_license(self, *args, **kwargs):
        """Validate license key."""
        return await self.provider.validate_license(*args, **kwargs)
    ...
    def list_available_plans(self) -> list:
        ...
        return [
            self.get_plan_details(plan_id)
            for plan_id in pricing_config.plans.keys()
        ]
    ...
    async def deduct_credits(self, license_key: str, amount: int, operation: str) -> dict:
        """
...
def get_payment_processor(provider: Optional[PaymentProvider] = None) -> PaymentProcessor:
