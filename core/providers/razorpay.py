        ...
        try:
            ...
            if payment_settings.trial_days > 0:
                subscription_data["start_at"] = int(
                    (datetime.utcnow() + timedelta(days=payment_settings.trial_days)).timestamp()
                )
            
            subscription = await self._make_request("POST", "/subscriptions", subscription_data)
    ...
    async def purchase_credits(
        ...
        try:
            ...
            if not pack:
                return PaymentResult(
                    success=False,
                    message=f"Credit pack not found: {credit_pack_id}"
                )
            ...
            license_key = self.generate_license_key(user_id, f"credits_{credit_pack_id}")
